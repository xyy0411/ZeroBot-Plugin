...
func init() {
	...
	engine.OnRegex(`^模拟([0-9]+)条消息`, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			var err error
			e := ctx.Event
			e.RawMessage = "虚拟事件"
	...
	engine.OnFullMatch("发送公告", zero.OnlyPrivate, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
					...
					case 1:
						...
						if msg == "确定" {
							...
							zero.RangeBot(func(id int64, ctx *zero.Ctx) bool {
								...
								for _, r := range ctx.GetFriendList().Array() {
									uid := r.Get("user_id").Int()
									ctx.SendPrivateMessage(uid, message.Text(origin))
									time.Sleep(1 * time.Second)
								}
								return true
							})
