...
type chatgptmodel struct {
	N        int    `db:"n"`
	Gptmodel string `db:"gptmodel"`
}

var (
	db    = &model{sql: sql.New(engine.DataFolder() + "chatgpt.db")}
	getdb = fcext.DoOnceOnSuccess(func(ctx *zero.Ctx) bool {
		...
		err = db.sql.Create("model", &chatgptmodel{})
		if err != nil {
			ctx.SendChain(message.Text("ERROR: ", err))
			return false
		}
...
func (db *model) seturl(n int, url string) error {
	db.Lock()
	defer db.Unlock()
	m := chatgptmodel{
		N:        n,
	...
	err := db.sql.Insert("model", &m)
	return err
}

func (db *model) findurl() (string, error) {
	db.Lock()
	defer db.Unlock()
	var m chatgptmodel
	err := db.sql.Find("model", &m, "where n = 1")
	if err != nil {
		return "", err
	}
	return m.Gptmodel, nil
}

func (db *model) insertmodel(uid int, gptmodel string) (err error) {
	db.Lock()
	defer db.Unlock()
	m := chatgptmodel{
		N:        uid,
...
func (db *model) findmodel(n int) (string, error) {
	db.Lock()
	defer db.Unlock()
	var m chatgptmodel
	err := db.sql.Find("model", &m, "where n = "+strconv.Itoa(n))
	if err != nil {
		return "", err
	}
	return m.Gptmodel, nil
